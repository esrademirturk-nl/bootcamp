/**
 * app/[locale]/bootcamps/page.tsx
 * Bootcamp Liste Sayfası (R3).
 * Filtreleme, arama, sıralama, sonuç sayısı indicator'ı, skeleton loading
...
// Eğer akraba yol (relative path) kullanarak içe aktarmak isterseniz:
import { BootcampFilters } from '../../../components/bootcamps/bootcamp-filters';
import { Skeleton } from '@/components/ui/skeleton';
import { Button } from '@/components/ui/button';
import Link from 'next/link';

// Page Props (Next.js App Router searchParams ve params alır)
interface PageProps {
...
/**
 * Yüklenme anında gösterilecek Skeleton (İskelet) Kart Listesi
 */
...
/**
 * Filtreleme ve sıralama mantığını çalıştıran iç bileşen
 */
async function BootcampListContent({
  ...
  let filtered = mockBootcamps.filter((bootcamp) => {
    ...
    // Kategori Filtresi (Çoklu seçim kontrolü)
  ...
  filtered = [...filtered].sort((a, b) => {
    ...
    // Varsayılan: Popülerlik / Puan
    return b.rating - a.rating;
  });

  return (
    <div className="space-y-6">
      {/* Result Count Indicator (Sonuç Sayısı Göstergesi) */}
      <div className="flex items-center justify-between pb-2 border-b border-border/40">
        <h2 className="text-lg font-semibold text-foreground">
          {filtered.length} {filtered.length === 1 ? 'Bootcamp' : 'Bootcamp'} Bulundu
        </h2>
      </div>

      {/* Empty State (Filtre sonucu boşsa gösterilecek alan) */}
      {filtered.length === 0 ? (
        ...
        /* Bootcamp Cards Grid (Kartların ızgara düzeni) */
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
...
export default async function BootcampsPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      {/* Ana Düzen: Sol Filtre Paneli + Sağ Liste Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        {/* Sağ Taraf: Kart Listesi ve Yüklenme Durumu */}
