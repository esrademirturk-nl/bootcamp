...
export default function ContactPage({ params }: ContactPageProps) {
  ...
  const validate = (values: FormValues): FormErrors => {
    ...
    if (!values.name.trim()) {
      newErrors.name = t('contactPage.errors.nameRequired', { defaultValue: 'Ad soyad alanı zorunludur.' });
    } else if (values.name.trim().length < 2) {
      newErrors.name = t('contactPage.errors.nameMin', { defaultValue: 'Ad en az 2 karakter olmalıdır.' });
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!values.email.trim()) {
      newErrors.email = t('contactPage.errors.emailRequired', { defaultValue: 'E-posta alanı zorunludur.' });
    } else if (!emailRegex.test(values.email.trim())) {
      newErrors.email = t('contactPage.errors.emailInvalid', { defaultValue: 'Geçerli bir e-posta adresi giriniz.' });
    }

    if (!values.subject.trim()) {
      newErrors.subject = t('contactPage.errors.subjectRequired', { defaultValue: 'Konu alanı zorunludur.' });
    } else if (values.subject.trim().length < 3) {
      newErrors.subject = t('contactPage.errors.subjectMin', { defaultValue: 'Konu en az 3 karakter olmalıdır.' });
    }

    if (!values.message.trim()) {
      newErrors.message = t('contactPage.errors.messageRequired', { defaultValue: 'Mesaj alanı zorunludur.' });
    } else if (values.message.trim().length < 10) {
      newErrors.message = t('contactPage.errors.messageMin', { defaultValue: 'Mesaj en az 10 karakter olmalıdır.' });
    }
    ...
    <div className="container mx-auto px-4 py-8 md:py-12 space-y-12 max-w-7xl">
      ...
      <div className="text-center space-y-3 max-w-2xl mx-auto">
        ...
        <p className="text-sm sm:text-base text-muted-foreground">
          {t('contactPage.subtitle', { defaultValue: 'Sorularınız, eğitim programları ve kayıt süreçleri için formu doldurabilirsiniz.' })}
        </p>
      </div>
          ...
          {isSuccess ? (
            <div className="text-center py-12 space-y-4">
              ...
              <p className="text-sm text-muted-foreground max-w-md mx-auto">
                {t('contactPage.successDesc', { defaultValue: 'Bizimle iletişime geçtiğiniz için teşekkür ederiz. En kısa sürede size dönüş yapacağız.' })}
              </p>
              <Button
                variant="secondary"
                className="mt-4"
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder={t('contactPage.placeholders.email', { defaultValue: 'john@example.com' })}
                  className={`w-full rounded-lg border bg-background px-4 py-2.5 text-sm text-foreground transition-colors focus:outline-none focus:ring-2 ${
                    errors.email
                      ? 'border-destructive focus:ring-destructive/30'
                      : 'border-border focus:border-primary focus:ring-primary/20'
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder={t('contactPage.placeholders.subject', { defaultValue: 'Bootcamp Kayıt Bilgisi' })}
                  className={`w-full rounded-lg border bg-background px-4 py-2.5 text-sm text-foreground transition-colors focus:outline-none focus:ring-2 ${
                    errors.subject
                      ? 'border-destructive focus:ring-destructive/30'
                      : 'border-border focus:border-primary focus:ring-primary/20'
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder={t('contactPage.placeholders.message', { defaultValue: 'Sorunuzu buraya detaylıca yazabilirsiniz...' })}
          ...
          <Card className="border-border/60 bg-card p-6 shadow-xs space-y-6">
            ...
            <div className="space-y-4 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground">{t('contactPage.addressLabel', { defaultValue: 'Adres' })}</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground">{t('contactPage.emailLabel', { defaultValue: 'E-posta' })}</p>
              ...
              <div className="flex items-start gap-3">
                <span className="text-xl">📞</span>
                <div>
                  <p className="font-semibold text-foreground">{t('contactPage.phoneLabel', { defaultValue: 'Telefon' })}</p>
                  <a href="tel:+31701234567" className="text-xs text-muted-foreground hover:text-foreground">
                    +31 70 123 4567
                  </a>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <span className="text-xl">🕒</span>
                <div>
                  <p className="font-semibold text-foreground">{t('contactPage.hoursLabel', { defaultValue: 'Çalışma Saatleri' })}</p>
                  <p className="text-xs text-muted-foreground">
                    {t('contactPage.hoursValue', { defaultValue: 'Pazartesi - Cuma: 09:00 - 18:00 (CET)' })}
                  </p>
                </div>
              </div>
            </div>
